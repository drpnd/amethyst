...
/* $Id: rbtree.c,v 572f48d68068 2010/11/09 03:49:40 Hirochika $ */
    ...
    int (*compare)(const void *, const void *))
{
    ...
    if ( 0 == ret ) {
        ...
        if ( _is_leaf((*node)->left) || _is_leaf((*node)->right) ) {
            ...
            assert( NULL != (*node)->right && NULL != (*node)->right->left );
...
_delete_one_child(struct rbtree_node **ptr)
{
    ...
    assert( _is_leaf(node->left) || _is_leaf(node->right) );
...
rbtree_init(struct rbtree *ptr, int (*compare)(const void *, const void *))
{
    if ( NULL == ptr ) {
        /* Allocate new */
        ptr = malloc(sizeof(struct rbtree));
        if ( NULL == ptr ) {
            /* Memory error */
            return NULL;
        }
...
{
    if ( NULL == iter ) {
        iter = malloc(sizeof(struct rbtree_iterator));
        if ( NULL == iter ) {
            /* Memory error */
            return NULL;
        }
